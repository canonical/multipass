...
fn generates_requested_word_number_with_separator() {
    let petname_1 = make_petname_generator(NumWords::One, '-' as i8)
        .unwrap()
        .make_name()
        .unwrap();
    let petname_2 = make_petname_generator(NumWords::Two, '-' as i8)
        .unwrap()
        .make_name()
        .unwrap();
    let petname_3 = make_petname_generator(NumWords::Three, '-' as i8)
        .unwrap()
        .make_name()
        .unwrap();
...
fn can_generate_unique_names() {
    ...
    let petname_generator = make_petname_generator(NumWords::Three, '-' as i8).unwrap();
