 ...
 * Hyper-V socket transport, addressing the guest by its VM id.
 */
struct HVSOCK
{
 ...
 *
 * When selected in SSHCoordinates::vsock_host, it directs the SSH session to use the
 * AF_VSOCK transport, addressing the peer by its context id.
 */
struct VSOCK
{
 ...
 * main interface used to create SSH sessions. It is produced by
 * BaseVirtualMachine::ssh_coordinates() and consumed by SSHFactory::make_session() to construct a
 * concrete SSHSession (e.g. PlainSSHSession), which uses these fields to set up the underlying
 * libssh connection. It can also be marshalled to/from the SSHCoordinatesInfo protobuf for gRPC
 * transport (see multipass::utils in grpc_utils.h).
 */
struct SSHCoordinates
{
    ...
    /// Base64-encoded private key used for public-key authentication.
    std::string private_key_as_base64;

    /// Port the guest's SSH server listens on (typically 22).
    uint32_t port;

    /// Hostname or IP address of the SSH server.
    std::string tcp_host;

    /// Optional vsock transport protocol.
