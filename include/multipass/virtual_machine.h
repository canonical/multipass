...
public:
    ...
    virtual std::shared_ptr<const Snapshot> take_snapshot(const QDir& dir, const VMSpecs& specs,
                                                          const std::string& name, const std::string& comment) = 0;
    virtual void load_snapshot(const QJsonObject& json) = 0; // TODO@snapshots remove
