...
public: // TODO@snapshots drop any accessors that we end up not needing
    ...
    virtual void shoot() = 0; // not using the constructor, we need snapshot objects for existing snapshots too
    virtual void delet() = 0; // not using the destructor, we want snapshots to stick around when daemon quits
};
} // namespace multipass
