...
|  `Zone`   |  The {ref}`availability zone <explanation-availability-zone>` the instance was launched into, as chosen automatically or via the `--zone` option of the {ref}`launch <reference-command-line-interface-launch>` command. If the zone has been disabled with {ref}`disable-zones <reference-command-line-interface-disable-zones>`, `(n/a)` is appended in table output.  |
