...
| **Unavailable** | The instance's {ref}`availability zone <explanation-availability-zone>` has been disabled with {ref}`disable-zones <reference-command-line-interface-disable-zones>`, so the instance has been forcefully switched off. It will start again automatically once its zone is re-enabled with {ref}`enable-zones <reference-command-line-interface-enable-zones>`. |
