...
TEST_F(ImageVault, downloads_image)
{
    ...
    auto vm_image =
        vault.fetch_image(mp::FetchType::ImageOnly, default_query, stub_prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, downloads_kernel_and_initrd)
{
    mp::DefaultVMImageVault vault{hosts, &url_downloader, cache_dir.path(), data_dir.path(), mp::days{0}};
    auto vm_image =
        vault.fetch_image(mp::FetchType::ImageKernelAndInitrd, default_query, stub_prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, calls_prepare)
{
    ...
    auto vm_image = vault.fetch_image(mp::FetchType::ImageOnly, default_query, prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, records_instanced_images)
{
    ...
    auto vm_image1 = vault.fetch_image(mp::FetchType::ImageOnly, default_query, prepare, stub_monitor, std::nullopt);
    auto vm_image2 = vault.fetch_image(mp::FetchType::ImageOnly, default_query, prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, caches_prepared_images)
{
    ...
    auto vm_image2 = vault.fetch_image(mp::FetchType::ImageOnly, another_query, prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, remembers_instance_images)
{
    ...
    auto vm_image1 =
        first_vault.fetch_image(mp::FetchType::ImageOnly, default_query, prepare, stub_monitor, std::nullopt);

    mp::DefaultVMImageVault another_vault{hosts, &url_downloader, cache_dir.path(), data_dir.path(), mp::days{0}};
    auto vm_image2 =
        another_vault.fetch_image(mp::FetchType::ImageOnly, default_query, prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, remembers_prepared_images)
{
    ...
    mp::DefaultVMImageVault another_vault{hosts, &url_downloader, cache_dir.path(), data_dir.path(), mp::days{0}};
    auto vm_image2 =
        another_vault.fetch_image(mp::FetchType::ImageOnly, another_query, prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, DISABLE_ON_WINDOWS_AND_MACOS(file_based_fetch_copies_image_and_returns_expected_info))
{
    ...
    auto vm_image = vault.fetch_image(mp::FetchType::ImageOnly, query, stub_prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, invalid_custom_image_file_throws)
{
    ...
    EXPECT_THROW(vault.fetch_image(mp::FetchType::ImageOnly, query, stub_prepare, stub_monitor, std::nullopt),
                 std::runtime_error);
}
...
TEST_F(ImageVault, DISABLE_ON_WINDOWS_AND_MACOS(custom_image_url_downloads))
{
    ...
    vault.fetch_image(mp::FetchType::ImageOnly, query, stub_prepare, stub_monitor, std::nullopt);
...
TEST_F(ImageVault, missing_downloaded_image_throws)
{
    ...
    EXPECT_THROW(vault.fetch_image(mp::FetchType::ImageOnly, default_query, stub_prepare, stub_monitor, std::nullopt),
