...
| `network` | An extra `--network` interface on an **ephemeral host bridge**: the guest still presents it with the same persisted MAC. Linux only. The bridge is created and destroyed *within each phase* (so it never lingers between runs); what survives the upgrade is the instance's NIC config (MAC + bridge reference), which verify re-attaches after re-creating the bridge. |
