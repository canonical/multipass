...
{
    LXDBackend()
        : mock_network_access_manager{std::make_unique<StrictMock<mpt::MockNetworkAccessManager>>()}
    {
...
TEST_F(LXDBackend, createsProjectStoragePoolAndNetworkOnHealthcheck)
{
    ...
    EXPECT_CALL(*mock_network_access_manager, createRequest(_, _, _))
        .WillRepeatedly([&project_created,
                         &network_created,
                         &storage_pool_created](auto, auto request, auto outgoingData) {
...
TEST_F(LXDBackend, usesMultipassStoragePoolWhenItExists)
{
    ...
    logger_scope.mock_logger->expect_log(mpl::Level::debug,
                                         "Using the \'multipass\' storage pool.");
...
TEST_F(LXDBackend, factoryCreatesExpectedImageVault)
{
    ...
    auto vault = backend.create_image_vault(hosts,
                                            &stub_downloader,
                                            cache_dir.path(),
                                            data_dir.path(),
                                            mp::days{0});
...
TEST_F(LXDBackend, machinePersistsInternalStoppedStateOnDestruction)
{
    ...
    EXPECT_CALL(mock_monitor, persist_state_for(_, _))
        .WillRepeatedly([&vm_instance_state](auto, auto state) { vm_instance_state = state; });
...
TEST_F(LXDBackend, prepareSourceImageDoesNotModify)
{
    mp::LXDVirtualMachineFactory backend{std::move(mock_network_access_manager),
                                         ...
                                         az_manager,
                                         base_url};
    const mp::VMImage original_image{"/path/to/image",
                                     "deadbeef",
                                     "bin",
                                     "baz",
                                     "the past",
...
TEST_F(LXDBackend, unimplementedFunctionsLogsTraceMessage)
{
    ...
    EXPECT_CALL(*logger_scope.mock_logger,
                log(Eq(mpl::Level::trace),
                    StrEq("lxd factory"),
                    StrEq("No driver preparation for instance image")));
...
TEST_F(LXDBackend, healthcheckThrowsWhenUntrusted)
{
    ...
    MP_EXPECT_THROW_THAT(backend.hypervisor_health_check(),
                         std::runtime_error,
...
INSTANTIATE_TEST_SUITE_P(LXDBackend,
                         LXDNetworkInfoSuite,
                         Values(mpt::network_leases_data,
...
TEST_F(LXDBackend, lxdRequestTimeoutAbortsAndThrows)
{
    ...
    const std::string error_string{fmt::format("Timeout getting response for {} operation on {}",
                                               op,
                                               base_url.toString().toStdString())};

    EXPECT_CALL(*logger_scope.mock_logger,
                log(Eq(mpl::Level::error), StrEq("lxd request"), HasSubstr(error_string)));

    MP_EXPECT_THROW_THAT(
        mp::lxd_request(mock_network_access_manager.get(), op, base_url, std::nullopt, 3),
        std::runtime_error,
...
TEST_F(LXDBackend, lxdRequestEmptyDataReturnedThrowsAndLogs)
{
    ...
    const std::string op{"GET"};
    const std::string error_string{fmt::format("Empty reply received for {} operation on {}",
                                               op,
                                               base_url.toString().toStdString())};
    ...
    MP_EXPECT_THROW_THAT(mp::lxd_request(mock_network_access_manager.get(), op, base_url),
                         std::runtime_error,
                         mpt::match_what(HasSubstr(error_string)));
}
...
TEST_F(LXDBackend, lxdRequestInvalidJsonThrowsAndLogs)
{
    ...
    MP_EXPECT_THROW_THAT(mp::lxd_request(mock_network_access_manager.get(), "GET", base_url),
                         std::runtime_error,
...
TEST_F(LXDBackend, lxdRequestWrongJsonThrowsAndLogs)
{
    ...
    EXPECT_CALL(*logger_scope.mock_logger,
                log(Eq(mpl::Level::debug),
                    StrEq("lxd request"),
                    AllOf(HasSubstr(base_url.toString().toStdString()),
...
TEST_F(LXDBackend, lxdRequestBadRequestThrowsAndLogs)
{
    EXPECT_CALL(*mock_network_access_manager, createRequest(_, _, _)).WillOnce([](auto...) {
        ...
        return new mpt::MockLocalSocketReply(error_data,
                                             QNetworkReply::ProtocolInvalidOperationError);
    });

    base_url.setHost("test");

    auto error_matcher = AllOf(HasSubstr("Network error for"),
                               HasSubstr(base_url.toString().toStdString()),
...
TEST_F(LXDBackend, lxdRequestMultipartBbadRequestThrowsAndLogs)
{
    ...
    EXPECT_CALL(*logger_scope.mock_logger,
                log(Eq(mpl::Level::error), StrEq("lxd request"), error_matcher));

    MP_EXPECT_THROW_THAT(
        mp::lxd_request(mock_network_access_manager.get(), "GET", base_url, stub_multipart),
        std::runtime_error,
        mpt::match_what(error_matcher));
}
...
TEST_F(LXDBackend, lxdWaitErrorReturnedThrowsAndLogs)
{
    EXPECT_CALL(*mock_network_access_manager, createRequest(_, _, _))
        .WillOnce([](auto, auto request, auto) {
            ...
            if (op == "GET" &&
                url.contains("1.0/operations/b043d632-5c48-44b3-983c-a25660d61164/wait"))
            {
    ...
    MP_EXPECT_THROW_THAT(
        mp::lxd_wait(mock_network_access_manager.get(), base_url, json_reply.object(), 1000),
        std::runtime_error,
        mpt::match_what(error_matcher));
}
...
TEST_F(LXDBackend, startWhileFrozenUnfreezes)
{
    ...
    EXPECT_CALL(*logger_scope.mock_logger,
                log(Eq(mpl::Level::info),
                    ...
                    StrEq("Resuming from a suspended state")));
...
TEST_F(LXDBackend, shutdownWhileStartingThrowsAndSetsCorrectState)
{
    ...
    MP_EXPECT_THROW_THAT(machine.ensure_vm_is_running(1ms),
                         mp::StartException,
...
TEST_F(LXDBackend, rebootsWhileStartingDoesNotThrowAndSetsCorrectState)
{
    ...
    EXPECT_CALL(*mock_network_access_manager, createRequest(_, _, _))
        .WillRepeatedly([&start_called, &running_returned, &reboot_simulated](auto,
                                                                              auto request,
                                                                              auto outgoingData) {
            outgoingData->open(QIODevice::ReadOnly);
            auto data = outgoingData->readAll();
...
INSTANTIATE_TEST_SUITE_P(LXDBackend,
                         LXDInstanceStatusTestSuite,
                         ValuesIn(lxd_instance_status_suite_inputs));
...
TEST_P(LXDNetworksOnlySupportedTypes, reportsOnlyBridgeAndEthernetNetworks)
{
    ...
    auto id_matcher = [](const std::string& expect) {
        return Field(&mp::NetworkInterfaceInfo::id, Eq(expect));
    };
    EXPECT_THAT(backend.networks(),
                AllOf(Each(Field(&mp::NetworkInterfaceInfo::type, MatchesRegex("bridge|ethernet"))),
                      UnorderedElementsAre(id_matcher("enxe4b97a832426"),
                                           id_matcher("lxdbr0"),
                                           id_matcher("mpbr0"),
                                           id_matcher("virbr0"),
                                           id_matcher("mpqemubr0"))));
}
...
TEST_F(LXDBackend, honorsBridgeDescriptionFromLxdWhenAvailable)
{
    ...
    auto data_template =
        QStringLiteral(R"({"metadata": [{"type": "bridge", "name": "br0", "description": "%1"}]})");
...
std::vector<Matcher<QJsonObject>> device_json_matchers_from(
    const std::vector<mp::NetworkInterface>& interfaces)
{
    std::vector<Matcher<QJsonObject>> device_matchers;
    for (const auto& interface : interfaces)
    {
        device_matchers.push_back(Truly([&interface](const QJsonObject& device) {
            return device_json_matches_interface(device, interface);
        }));
    }
                                    ...
                                    Matcher<QIODevice*> request_contents_matcher)
{
    EXPECT_CALL(mock_network_access_mgr,
                createRequest(QNetworkAccessManager::CustomOperation,
                              custom_request_matcher("GET", "pied-piper-valley/state"),
                              _))
        .Times(3)
    ...
    EXPECT_CALL(
        mock_network_access_mgr,
        createRequest(
            QNetworkAccessManager::CustomOperation,
            custom_request_matcher("GET", "operations/0020444c-2e4c-49d5-83ed-3275e3f6d005/wait"),
            _))
...
TEST_F(LXDBackend, postsExtraNetworkDevices)
{
    ...
    auto devices_matcher =
        IsSupersetOf(device_json_matchers_from(default_description.extra_interfaces));
    auto json_matcher = ResultOf(&extract_devices, devices_matcher);
    setup_vm_creation_expectations(*mock_network_access_manager,
                                   request_data_matcher(json_matcher));
...
{
    CustomLXDFactory backend{std::move(mock_network_access_manager),
                             data_dir.path(),
                             az_manager,
                             base_url};
...
TEST_F(LXDBackend, createsBridgesViaBackendUtils)
{
    ...
    CustomLXDFactory factory{std::move(mock_network_access_manager),
                             data_dir.path(),
                             az_manager,
                             base_url};
