...
{
    const mpt::MockFileOps::GuardedMock guarded_mock_file_ops = mpt::MockFileOps::inject<NaggyMock>();
    mpt::MockFileOps& mock_file_ops = *guarded_mock_file_ops.first;
    const mpt::MockPlatform::GuardedMock guarded_mock_platform = mpt::MockPlatform::inject<NaggyMock>();
...
TEST_F(TestPermissionUtilsFile, restrict_permissions_throws_on_perms_failure)
{
    EXPECT_CALL(mock_platform, set_permissions(test_path, restricted_permissions, _)).WillOnce(Return(false));
...
TEST_F(TestPermissionUtilsFile, restrict_permissions_throws_on_owner_failure)
{
