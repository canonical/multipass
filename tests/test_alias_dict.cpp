...
TEST_F(AliasDictionary, skips_correctly_broken_entries)
{
    ...
    ASSERT_EQ(a1->map_working_directory, true);

    auto a2 = dict.get_alias("alias2");
    ASSERT_TRUE(a2);
    ASSERT_EQ(a2->instance, "second_instance");
    ASSERT_EQ(a2->command, "second_command");
    ASSERT_EQ(a2->map_working_directory, false);
}

// In old versions, the file did not contain the `map-working-directory` flag in the JSON, because the
// `--no-map-working-directory` flag were not yet introduced. In case the file was generated by an old version,
// the flag must be `false`, in order to maintain backwards compatibility.
TEST_F(AliasDictionary, mapDirInOldFilesDefaultsToFalse)
{
    std::string file_contents{"{\n"
                              "    \"alias3\": {\n"
    ...
    ASSERT_EQ(a3->map_working_directory, false);
}
