...
void write_source_record(const fs::path& data_dir, const std::string& name)
{
    ...
    const boost::json::object records{{name, boost::json::value_from(image_record(
                                                  data_dir / "vault" / "instances" / name /
                                                  "source.vhdx"))}};
...
void write_prepared_target(const fs::path& dir, const std::string& name)
{
    ...
    const mhv::MigrationTransactionManifest manifest{
        ...
        .phase = mhv::MigrationTransactionManifest::prepared_phase_name,
        .vm_name = name,
        .active_disk = dir / "active.avhdx",
        .state_file_stem = dir / "hcs-state",
        .owned_disks = {dir / "active.avhdx"},
    };
...
TEST(HyperVMigrationTargetRecords, commitsImageBeforeVisibleVmAndRemovesManifest)
{
    ...
    EXPECT_TRUE(store.has_vm_record("vm"));
    EXPECT_TRUE(store.has_image_record("vm"));
    EXPECT_FALSE(MP_FILEOPS.exists(target_dir /
                                   mhv::MigrationTransactionManifest::filename));

    const auto vm_records =
        read_records(store.target_data_root() / "multipassd-vm-instances.json");
    const auto image_records = read_records(
        store.target_data_root() / "vault" /
        "multipassd-instance-image-records.json");
    EXPECT_TRUE(vm_records.contains("vm"));
    ASSERT_TRUE(image_records.contains("vm"));
    EXPECT_EQ(boost::json::value_to<std::string>(
                  image_records.at("vm").at("image").at("path")),
...
TEST(HyperVMigrationTargetRecords, recoveryRemovesOrphanImageRecordAndDirectory)
{
    ...
    EXPECT_FALSE(MP_FILEOPS.exists(target_dir));
    EXPECT_FALSE(store.has_image_record("orphan"));
    const auto persisted = read_records(
        target_root / "vault" / "multipassd-instance-image-records.json");
