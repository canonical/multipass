...
struct HighLevelFileOps : public Test
{
    HighLevelFileOps()
    {
        ...
        EXPECT_CALL(mock_file_ops, read_all(A<const fs::path&>()))
            .WillRepeatedly([this]<typename... Args>(Args&&... args) {
                return mock_file_ops.FileOps::read_all(std::forward<Args>(args)...);
            });
    }
...
TEST_F(HighLevelFileOps, readAllReadsFromFile)
{
    ...
    *filestream << "Hello, world!";
    ...
    const auto filedata = mock_file_ops.read_all(":)");

    EXPECT_EQ(filedata, "Hello, world!");
}

TEST_F(HighLevelFileOps, readAllThrowsOnFailbit)
{
    auto filestream = std::make_unique<std::stringstream>();
    ...
    EXPECT_THROW(mock_file_ops.read_all(":("), std::ios_base::failure);
}

TEST_F(HighLevelFileOps, readAllThrowsOnBadbit)
{
    auto filestream = std::make_unique<std::stringstream>();
