...
struct QemuBackend : public mpt::TestWithMockedBinPath
{
    QemuBackend()
    {
        ...
        EXPECT_CALL(*mock_qemu_platform, full_platform_args(_)).WillRepeatedly(Return(QStringList()));
    };
...
TEST(QemuPlatform, base_qemu_platform_returns_expected_values)
{
    ...
    EXPECT_CALL(qemu_platform, base_platform_args()).WillOnce([&qemu_platform] {
        return qemu_platform.QemuPlatform::base_platform_args();
    });
    EXPECT_CALL(qemu_platform, full_platform_args(_)).WillOnce([&qemu_platform](auto& vm_desc) {
        return qemu_platform.QemuPlatform::full_platform_args(vm_desc);
    });
    EXPECT_CALL(qemu_platform, get_directory_name()).WillOnce([&qemu_platform] {
        return qemu_platform.QemuPlatform::get_directory_name();
    });

    EXPECT_TRUE(qemu_platform.base_platform_args().isEmpty());
    EXPECT_TRUE(qemu_platform.full_platform_args(vm_desc).isEmpty());
