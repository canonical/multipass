...
TEST_P(DaemonCreateLaunchTestSuite, blueprint_not_found_passes_expected_data)
{
    ...
    EXPECT_CALL(*mock_image_vault, fetch_image(_, _, _, _, _)).WillOnce(mpt::fetch_image_lambda(release, empty));
...
TEST_F(Daemon, ctor_drops_removed_instances)
{
    ...
    auto mock_image_vault = std::make_unique<NiceMock<mpt::MockVMImageVault>>();
    EXPECT_CALL(*mock_image_vault, fetch_image(_, Field(&mp::Query::name, stayed), _, _, _))
        .WillRepeatedly(DoDefault()); // returns an image that can be verified to exist for this instance
    EXPECT_CALL(*mock_image_vault, fetch_image(_, Field(&mp::Query::name, gone), _, _, _))
        .WillOnce(
