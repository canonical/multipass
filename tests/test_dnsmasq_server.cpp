...
#include <src/platform/confinement/unconfined_system.h>
...
struct DNSMasqServer : public mpt::TestWithMockedBinPath
{
    ...
    std::shared_ptr<multipass::UnconfinedSystem> confinement = std::make_shared<multipass::UnconfinedSystem>();
...
{
    EXPECT_NO_THROW(mp::DNSMasqServer dns(confinement, data_dir.path(), bridge_name, bridge_addr, start_addr, end_addr));
}

TEST_F(DNSMasqServer, finds_ip)
{
    mp::DNSMasqServer dns{confinement, data_dir.path(), bridge_name, bridge_addr, start_addr, end_addr};
    make_lease_entry();
...
TEST_F(DNSMasqServer, release_mac_releases_ip)
{
    ...
    mp::DNSMasqServer dns{confinement, data_dir.path(), dchp_release_called, bridge_addr, start_addr, end_addr};
    make_lease_entry();
