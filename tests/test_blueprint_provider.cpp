...
{
    QString blueprints_zip_url{QUrl::fromLocalFile(mpt::test_data_path()).toString() +
                               test_blueprints_zip};
...
TEST_F(VMBlueprintProvider, fetchBlueprintForUnknownBlueprintThrows)
{
    ...
    EXPECT_THROW(blueprint_provider.fetch_blueprint_for("phony", vm_desc, dummy_data),
                 std::out_of_range);
}
...
TEST_F(VMBlueprintProvider, invalidImageSchemeThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.fetch_blueprint_for("invalid-image-blueprint", vm_desc, dummy_data),
...
TEST_F(VMBlueprintProvider, invalidMinCoresThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.fetch_blueprint_for("invalid-cpu-blueprint", vm_desc, dummy_data),
        mp::InvalidBlueprintException,
...
TEST_F(VMBlueprintProvider, invalidMinMemorySizeThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.fetch_blueprint_for("invalid-memory-size-blueprint",
                                               vm_desc,
                                               dummy_data),
        mp::InvalidBlueprintException,
...
TEST_F(VMBlueprintProvider, invalidMinDiskSpaceThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.fetch_blueprint_for("invalid-disk-space-blueprint", vm_desc, dummy_data),
        mp::InvalidBlueprintException,
...
TEST_F(VMBlueprintProvider, invalidAliasDefinitionThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.fetch_blueprint_for("invalid-alias-blueprint", vm_desc, dummy_data),
        mp::InvalidBlueprintException,
...
TEST_F(VMBlueprintProvider, missingDescriptionThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.info_for(blueprint),
        mp::InvalidBlueprintException,
        mpt::match_what(StrEq(
            fmt::format("The \'description\' key is required for the {} Blueprint", blueprint))));
}
...
TEST_F(VMBlueprintProvider, missingVersionThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        ...
        mp::InvalidBlueprintException,
        mpt::match_what(
            StrEq(fmt::format("The \'version\' key is required for the {} Blueprint", blueprint))));
}
...
TEST_F(VMBlueprintProvider, invalidDescriptionThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        ...
        mp::InvalidBlueprintException,
        mpt::match_what(StrEq(
            fmt::format("Cannot convert \'description\' key for the {} Blueprint", blueprint))));
}
...
TEST_F(VMBlueprintProvider, invalidVersionThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        ...
        mp::InvalidBlueprintException,
        mpt::match_what(
            StrEq(fmt::format("Cannot convert \'version\' key for the {} Blueprint", blueprint))));
}
...
TEST_F(VMBlueprintProvider, invalidCloudInitThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.fetch_blueprint_for(blueprint, vm_desc, dummy_data),
        mp::InvalidBlueprintException,
        mpt::match_what(
            StrEq(fmt::format("Cannot convert cloud-init data for the {} Blueprint", blueprint))));
}
...
TEST_F(VMBlueprintProvider, allBlueprintsReturnsExpectedInfo)
{
    ...
    logger_scope.mock_logger->expect_log(mpl::Level::error,
                                         "Invalid Blueprint: Cannot convert 'description' key for "
                                         "the invalid-description-blueprint Blueprint");
    logger_scope.mock_logger->expect_log(mpl::Level::error,
                                         "Invalid Blueprint: Cannot convert 'version' key for the "
                                         "invalid-version-blueprint Blueprint");
    logger_scope.mock_logger->expect_log(mpl::Level::error,
                                         "Invalid Blueprint: The 'description' key is required for "
                                         "the missing-description-blueprint Blueprint");
    logger_scope.mock_logger->expect_log(mpl::Level::error,
                                         "Invalid Blueprint: The 'version' key is required for the "
                                         "missing-version-blueprint Blueprint");
    logger_scope.mock_logger->expect_log(
        mpl::Level::error,
        "Invalid Blueprint name \'42-invalid-hostname-blueprint\': must be a valid host name");
    logger_scope.mock_logger->expect_log(mpl::Level::debug,
                                         "Not loading malformed \"invalid-arch\" v1");
    ...
    EXPECT_TRUE(std::find_if(blueprints.cbegin(),
                             blueprints.cend(),
                             [](const mp::VMImageInfo& blueprint_info) {
                                 return (
                                     (blueprint_info.aliases.size() == 1) &&
                                     (blueprint_info.aliases[0] == "test-blueprint1") &&
    ...
    EXPECT_TRUE(std::find_if(blueprints.cbegin(),
                             ...
                             [](const mp::VMImageInfo& blueprint_info) {
                                 return (
                                     (blueprint_info.aliases.size() == 1) &&
                                     (blueprint_info.aliases[0] == "test-blueprint2") &&
...
TEST_F(VMBlueprintProvider, downloadFailureOnStartupLogsErrorAndDoesNotThrow)
{
    ...
    mpt::MockURLDownloader mock_url_downloader;
    EXPECT_CALL(mock_url_downloader, download_to(_, _, _, _, _))
        .WillOnce(Throw(mp::DownloadException(url, error_msg)));

    auto logger_scope = mpt::MockLogger::inject();
    logger_scope.mock_logger->screen_logs(mpl::Level::error);
    logger_scope.mock_logger->expect_log(
        mpl::Level::error,
        fmt::format("Error fetching Blueprints: failed to download from '{}': {}", url, error_msg));

    EXPECT_NO_THROW(mp::DefaultVMBlueprintProvider(blueprints_zip_url,
                                                   &mock_url_downloader,
                                                   cache_dir.path(),
                                                   default_ttl));
}
...
TEST_F(VMBlueprintProvider, zipArchivePocoExceptionLogsErrorAndDoesNotThrow)
{
    ...
    EXPECT_CALL(*mock_poco_zip_utils, zip_archive_for(_))
        .WillOnce(Throw(Poco::IllegalStateException(error_msg)));

    auto logger_scope = mpt::MockLogger::inject();
    logger_scope.mock_logger->screen_logs(mpl::Level::error);
    logger_scope.mock_logger->expect_log(
        mpl::Level::error,
        fmt::format("Error extracting Blueprints zip file: Illegal state: {}", error_msg));
...
TEST_F(VMBlueprintProvider, generalExceptionDuringStartupThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        mp::DefaultVMBlueprintProvider blueprint_provider(blueprints_zip_url,
                                                          ...
                                                          std::chrono::milliseconds(0)),
        std::runtime_error,
        mpt::match_what(StrEq(error_msg)));
}
...
TEST_F(VMBlueprintProvider, generalExceptionDuringCallThrows)
{
    ...
    MP_EXPECT_THROW_THAT(blueprint_provider.info_for("foo"),
                         std::runtime_error,
                         mpt::match_what(StrEq(error_msg)));
}
...
TEST_F(VMBlueprintProvider, nameMismatchThrows)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.fetch_blueprint_for("name-mismatch", vm_desc, dummy_data),
        mp::InvalidBlueprintException,
        mpt::match_what(
            StrEq("There are no instance definitions matching Blueprint name \"name-mismatch\"")));
}
...
TEST_F(VMBlueprintProvider, fetchForInvalidReturnsEmpty)
{
    ...
    // This call fails with an std::nullopt exception because the Blueprint is invalid and was
    // filtered out by blueprints_map_for() at provider construction.
...
TEST_F(VMBlueprintProvider, allBlueprintsReturnsExpectedInfoForArch)
{
    ...
    EXPECT_TRUE(std::find_if(blueprints.cbegin(),
                             ...
                             [](const mp::VMImageInfo& blueprint_info) {
                                 return (
                                     (blueprint_info.aliases.size() == 1) &&
                                     (blueprint_info.aliases[0] == "arch-only") &&
...
TEST_F(VMBlueprintProvider, v2ShaOnUrlIsCorrectlyPropagated)
{
    ...
    auto query =
        blueprint_provider.fetch_blueprint_for("minimal-with-sha256-url", vm_desc, launch_data);
...
TEST_F(VMBlueprintProvider, v2HardcodedShaIsCorrectlyPropagated)
{
    ...
    auto query =
        blueprint_provider.fetch_blueprint_for("minimal-with-sha256-string", vm_desc, launch_data);
...
struct VMBlueprintFileLaunchFromFile
    : public VMBlueprintFileLaunch,
...
TEST_P(VMBlueprintFileLaunchFromFile, loadsFile)
{
    ...
    auto blueprint_path =
        QString(mpt::test_data_path() + "/blueprints/" + QString::fromStdString(file))
            .toStdString();
    ...
    auto query =
        blueprint_provider.blueprint_from_file(blueprint_path, blueprint_name, vm_desc, dummy_data);
...
INSTANTIATE_TEST_SUITE_P(VMBlueprintFileLaunch,
                         VMBlueprintFileLaunchFromFile,
...
TEST_F(VMBlueprintFileLaunch, mergesBlueprintVendorData)
{
    ...
    blueprint_provider.blueprint_from_file(blueprint_path.toStdString(),
                                           "complex-cloud-init",
                                           vm_desc,
                                           dummy_data);
...
TEST_F(VMBlueprintFileLaunch, failsMergeVmBlueprintVendorDataDifferentTypes)
{
    ...
    MP_EXPECT_THROW_THAT(blueprint_provider.blueprint_from_file(blueprint_path.toStdString(),
                                                                "complex-cloud-init",
                                                                vm_desc,
                                                                dummy_data),
                         mp::InvalidBlueprintException,
...
TEST_F(VMBlueprintFileLaunch, failsWithNonexistentFile)
{
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.blueprint_from_file("/blah.yaml", "blah", vm_desc, dummy_data),
        mp::InvalidBlueprintException,
        mpt::match_what(StrEq("Wrong file '/blah.yaml'")));
}

TEST_F(VMBlueprintFileLaunch, fileLoadfailsWithInvalidHostName)
{
    auto blueprint_path =
        QString(mpt::test_data_path() + "/blueprints/v1/42-invalid-hostname-blueprint.yaml")
            .toStdString();
    ...
    MP_EXPECT_THROW_THAT(
        blueprint_provider.blueprint_from_file(blueprint_path,
                                               "42-invalid-hostname-blueprint",
                                               vm_desc,
                                               dummy_data),
        mp::InvalidBlueprintException,
        mpt::match_what(StrEq("Invalid Blueprint name \'42-invalid-hostname-blueprint\': must be a "
                              "valid host name")));
}
...
INSTANTIATE_TEST_SUITE_P(VMBlueprintProvider,
                         NameFromBlueprintTestSuite,
