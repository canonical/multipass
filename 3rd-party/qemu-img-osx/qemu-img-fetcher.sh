...
get_brew_and_extract qemu-2.9.0

cp "$DOWNLOAD_DIR/qemu/2.9.0/bin/qemu-img" "$BINDIR/"
...
# glib - requres pcre
get_brew_and_extract glib-2.52.3

cp "$DOWNLOAD_DIR/glib/2.52.3/lib/libgthread-2.0.0.dylib" "$LIBDIR/"
cp "$DOWNLOAD_DIR/glib/2.52.3/lib/libglib-2.0.0.dylib"    "$LIBDIR/"

# pcre - required by glib (no other deps)
get_brew_and_extract pcre-8.41

cp "$DOWNLOAD_DIR/pcre/8.41/lib/libpcre.1.dylib" "$LIBDIR/"

# gettext (no other deps)
get_brew_and_extract gettext-0.19.8.1

cp "$DOWNLOAD_DIR/gettext/0.19.8.1/lib/libintl.8.dylib" "$LIBDIR/"

# nettle (no other deps)
get_brew_and_extract nettle-3.3

cp "$DOWNLOAD_DIR/nettle/3.3/lib/libnettle.6.dylib"  "$LIBDIR/"
cp "$DOWNLOAD_DIR/nettle/3.3/lib/libhogweed.4.dylib" "$LIBDIR/"  # needed by gnutls

# gnutls - requires p11-kit, libunistring, libtasn1, nettle (above), gmp
get_brew_and_extract gnutls-3.5.12_2

cp "$DOWNLOAD_DIR/gnutls/3.5.12_2/lib/libgnutls.30.dylib" "$LIBDIR/"

# p11-kit - requires libffi
get_brew_and_extract p11-kit-0.23.7

cp "$DOWNLOAD_DIR/p11-kit/0.23.7/lib/libp11-kit.0.dylib" "$LIBDIR/"
...
get_brew_and_extract libunistring-0.9.7

cp "$DOWNLOAD_DIR/libunistring/0.9.7/lib/libunistring.2.dylib" "$LIBDIR/"

# libtasn1 (no other deps)
get_brew_and_extract libtasn1-4.9

cp "$DOWNLOAD_DIR/libtasn1/4.9/lib/libtasn1.6.dylib" "$LIBDIR/"

# gmp (no other deps)
get_brew_and_extract gmp-6.1.2

cp "$DOWNLOAD_DIR/gmp/6.1.2/lib/libgmp.10.dylib" "$LIBDIR/"
...
install_name_tool -change "@@HOMEBREW_CELLAR@@/glib/2.52.3/lib/libglib-2.0.0.dylib" "@rpath/libglib-2.0.0.dylib"      "$LIBDIR/libgthread-2.0.0.dylib"
...
install_name_tool -change "@@HOMEBREW_CELLAR@@/nettle/3.3/lib/libnettle.6.dylib"    "@rpath/libnettle.6.dylib"        "$LIBDIR/libhogweed.4.dylib"
