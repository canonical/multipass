...
mp::ReturnCode cmd::Launch::request_launch()
{
    ...
    auto on_failure = [this, &spinner](grpc::Status& status) {
        ...
        for (const auto& error : launch_error.error_codes())
        {
            ...
            {
                error_details =
                    fmt::format("Invalid disk size value supplied: {}.", request.disk_space(), min_disk_size);
            }
            else if (error == LaunchError::INVALID_MEM_SIZE)
            {
                error_details =
                    fmt::format("Invalid memory size value supplied: {}.", request.mem_size(), min_memory_size);
            }
