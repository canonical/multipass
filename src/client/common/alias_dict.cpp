...
mp::AliasDict mp::AliasDict::load_file(mp::Terminal* term, const std::string& filename)
{
    ...
    try
    {
        json = boost::json::parse(std::string_view(db_file.readAll()));
    }
...
{
    sanitize_contexts();
    MP_JSONUTILS.write_json(boost::json::value_from(*this), QString::fromStdString(aliases_file));
}
                    ...
                    const mp::AliasDict& alias_dict)
{
    json = {{"active-context", alias_dict.active_context},
            {"contexts", boost::json::value_from(alias_dict.aliases)}};
}
