...
std::string mp::TableFormatter::format(const InfoReply& reply) const
{
    ...
    for (const auto& info : format::sorted(reply.info()))
    {
        ...
        for (auto mount = mount_paths.cbegin(); mount != mount_paths.cend(); ++mount)
        {
            ...
            {
                fmt::format_to(std::back_inserter(buf), "{:>{}}{}:{}{}{}",
                               (uid_map == mount->mount_maps().uid_map().cbegin()) ? "UID map: " : "",
                               (uid_map == mount->mount_maps().uid_map().cbegin()) ? 29 : 0,
                               std::to_string(uid_map->first),
            ...
            {
                fmt::format_to(std::back_inserter(buf), "{:>{}}{}:{}{}{}",
                               (gid_map == mount->mount_maps().gid_map().cbegin()) ? "GID map: " : "",
                               (gid_map == mount->mount_maps().gid_map().cbegin()) ? 29 : 0,
                               std::to_string(gid_map->first),
