...
mp::ReturnCode cmd::BlockDetach::run(mp::ArgParser* parser)
{
    ...
    return dispatch(&RpcMethod::detach_block, request, on_success, on_failure);
}

std::string cmd::BlockDetach::name() const
{
    return "block-detach";
}

QString cmd::BlockDetach::short_help() const
{
    return QStringLiteral("Detach a block device from a VM");
}
