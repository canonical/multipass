...
{
    mpl::trace(category, fmt::format("Loading client certs from {}", cert_dir.absolutePath()));
}

void mp::ClientCertStore::add_cert(const std::string& pem_cert)
{
    mpl::trace(category, fmt::format("Adding cert:\n{}", pem_cert));
...
{
    mpl::trace(category, fmt::format("Verifying cert:\n{}", pem_cert));
