...
try // clang-format on
{
    mpl::ClientLogger<ZonesReply, ZonesRequest> logger{mpl::level_from(request->verbosity_level()),
                                                       *config->logger,
                                                       server};
    ...
    {
        auto reply_zone = response.add_zones();
...
try // clang-format on
{
    mpl::ClientLogger<ZonesStateReply, ZonesStateRequest> logger{mpl::level_from(request->verbosity_level()),
                                                                 *config->logger,
                                                                 server};

    ZonesStateReply response{};
    ...
    auto make_vm_description = [this, server, request, name, checked_args, log_level]() mutable -> VMFullDescription {
        ...
        try
        {
            ...
            VirtualMachineDescription vm_desc{
                ...
                name,
                // TODO USE ALL ZONES, LAUNCHING ONE VM PER ZONE INSTEAD OF JUST LAUNCHING ONE IN THE FIRST ZONE
                request->zones().empty() ? config->az_manager->get_automatic_zone_name() : *request->zones().begin(),
                "",
                {},
                config->ssh_username,
                VMImage{},
