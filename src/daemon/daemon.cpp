...
std::unordered_map<std::string, mp::VMSpecs> load_db(const mp::Path& data_path, const mp::Path& cache_path)
{
    ...
    for (auto it = records.constBegin(); it != records.constEnd(); ++it)
    {
        ...
        reconstructed_records[key] = {num_cores,
                                      mp::MemorySize{mem_size.empty() ? multipass::default_memory_size : mem_size},
                                      mp::MemorySize{disk_space.empty() ? multipass::default_disk_size : disk_space},
                                      mac_addr,
                                      ssh_username,
...
auto validate_create_arguments(const mp::LaunchRequest* request)
{
    ...
    const auto opt_mem_size = try_mem_size(mem_size_str.empty() ? multipass::default_memory_size : mem_size_str);
    const auto opt_disk_space = try_mem_size(disk_space_str.empty() ? multipass::default_disk_size : disk_space_str);
