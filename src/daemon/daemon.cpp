                                                     ...
                                                     const mp::Path& cache_path)
{
    ...
    for (const auto& [key, record] : records.as_object())
    {
        ...
        auto specs = value_to<mp::VMSpecs>(record);
        // FIXME: Check that ssh_username is empty too, like before?
        if (!specs.num_cores && !specs.deleted && specs.metadata.empty() &&
            !specs.mem_size.in_bytes() && !specs.disk_space.in_bytes())
        {
            mpl::warn(category, "Ignoring ghost instance in database: {}", key);
            continue;
        }

        reconstructed_records.emplace(key, specs);
    }
                                   ...
                                   const std::string& dest_name)
{
    ...
    if (!dest_vm_spec.metadata.empty())
    {
        dest_vm_spec.metadata =
            MP_JSONUTILS.update_unique_identifiers_of_metadata(dest_vm_spec.metadata,
                                                               src_vm_spec,
                                                               dest_vm_spec,
                                                               src_name,
                                                               dest_name);
