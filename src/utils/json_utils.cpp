...
// Pretty print a JSON value non-recursively. The general strategy here is to maintain a stack of
// counted iterators representing our progress through each nested container (arrays or objects).
// While processing a particular container, we can immediately print any scalars we encounter, but
// if we hit another container, we push its iterator onto the stack and break out of our inner loop
// to start processing that new container.
void mp::pretty_print(std::ostream& os,
                      const boost::json::value& value,
                      const PrettyPrintOptions& opts)
{
    std::stack<some_iter_t> stack;
    ...
    outer:
        os.put('\n');
        if (std::holds_alternative<object_iter_t>(stack.top()))
        {
            auto& i = std::get<object_iter_t>(stack.top());
        ...
        {
            auto& i = std::get<array_iter_t>(stack.top());
            while (i != std::default_sentinel)
            {
                indent(os, opts.indent * stack.size());
