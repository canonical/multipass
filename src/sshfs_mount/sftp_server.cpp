...
bool mp::SftpServer::validate_path(const fs::path& current_path, bool follows_symlinks) const
{
    ...
    try
    {
        ...
        if (follows_symlinks)
        {
            ...
            while (check_path != check_path.parent_path() && !fs::exists(check_path))
            {
...
int mp::SftpServer::handle_mkdir(sftp_client_message msg)
{
    ...
    QDir dir(QString::fromStdString(filename->string()));
    QFileInfo current_dir(QString::fromStdString(filename->string()));
...
int mp::SftpServer::handle_readdir(sftp_client_message msg)
{
    ...
    for (int i = 0; i < max_num_entries_per_packet && dir_iterator.hasNext(); i++)
    {
        ...
        QFileInfo file_info(QString::fromStdString(entry.path().string()));
        sftp_attributes_struct attr{};
        if (entry.is_symlink())
        {
...
int mp::SftpServer::handle_rename(sftp_client_message msg)
{
    ...
    QFileInfo source_info{QString::fromStdString(source->string())};
...
int mp::SftpServer::handle_symlink(sftp_client_message msg)
{
    ...
    QFileInfo file_info{QString::fromStdString(old_name->string())};
    if (MP_FILEOPS.exists(file_info) && !has_id_mappings_for(file_info))
    {
        mpl::log(mpl::Level::trace,
                 category,
    ...
    if (!MP_PLATFORM.symlink(old_name->string().c_str(),
                             ...
                             QFileInfo(QString::fromStdString(old_name->string())).isDir()))
    {
        mpl::log(mpl::Level::trace,
                 category,
