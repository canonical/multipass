...
namespace
{
constexpr static auto kLogCategory = "apple-virtual-machine";
}
...
{
    vm_handle.reset();
    if (!MP_APPLE_VZ.create_vm(desc, vm_handle))
    {
        mpl::warn(kLogCategory, "Failed to create handle for VM: `{}`!", vm_name);
    }
}
