...
{
    PowerShell::run_once({"Remove-VM", "-Name", QString::fromStdString(name), "-Force"}, name);
}
                                                             ...
                                                             const VirtualMachineDescription& desc)
{
    if (!desc.disk_space.empty())
    {
        ...
        if (disk_space.endsWith('K') || disk_space.endsWith('M') || disk_space.endsWith('G'))
            disk_space.append("B");
        PowerShell::run_once({"Resize-VHD", "-Path", instance_image.image_path, "-SizeBytes", disk_space},
                             desc.vm_name);
    }
}
