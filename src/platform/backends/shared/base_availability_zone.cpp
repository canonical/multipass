...
BaseAvailabilityZone::data BaseAvailabilityZone::make(const std::string& name, const fs::path& az_directory)
{
    ...
    if (std::error_code err{}; MP_FILEOPS.exists(file_path, err))
        return read_from_file(name, file_path);
    else if (err)
        throw FormattedExceptionBase{"AZ file '{}' is not accessible: {}", file_path, err.message()};

    mpl::debug(name, "AZ file '{}' not found, using defaults", file_path);
    return {
        // TODO remove these comments in C++20
        /*.name = */ name,
        /*.file_path = */ file_path,
