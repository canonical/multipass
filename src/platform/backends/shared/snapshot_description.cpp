                    ...
                    const mp::SnapshotDescription& desc)
{
    json = {
        ...
        {"mounts", boost::json::value_from(desc.mounts, map_as_array{"target_path"})},
                                       ...
                                       const snapshot_context& ctx)
{
    ...
    return {value_to<std::string>(json.at("name")),
            ...
            QDateTime::fromString(
                QString::fromStdString(value_to<std::string>(json.at("creation_timestamp"))),
                Qt::ISODateWithMs),
