...
sid_buffer get_user_sid(const std::wstring& user_name)
{
    ...
    if (!LookupAccountNameW(nullptr, user_name.c_str(), sid.data(), &sid_size, domain.data(), &domain_size, &sid_use))
...
bool has_full_control(const std::filesystem::path& path, sid_buffer& user_sid)
{
    ...
    for (DWORD i = 0; i < pDACL->AceCount; ++i)
    {
        ...
        if (!EqualSid(reinterpret_cast<PSID>(&ace->SidStart), reinterpret_cast<PSID>(user_sid.data())))
            continue;
