      ...
      monitor{&monitor}
{
    ...
    // 1. clone the vm with certain options and mode. --mode value is all, which copied all snapshot history and it
    // includes the base disk whereas machine mode only copies the current differencing disk when the vm is snapshoted.
    // --options have keepdisknames and keepallmacs, keepdisknames means disk file name will be kept instead using
    // <vm_name>.vdi, keepallmacs imply that the mac addresses of network adapters will not be generated by virtualbox
    // because we will replace them with multipass generated mac addresses anyway.
    mpu::process_throw_on_error("VBoxManage",
                                {"clonevm",
