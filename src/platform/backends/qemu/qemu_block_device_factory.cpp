                                                                      ...
                                                                      const Path& data_dir)
{
    ...
    if (size < MemorySize{"1G"})
        throw ValidationError(
            fmt::format("Block device size must be at least 1G, got '{}'", size.in_bytes()));
...
mp::BlockDevice::UPtr mp::QemuBlockDeviceFactory::create_block_device_from_file(
    ...
    const Path& data_dir)
{
    ...
    if (!source_info.exists())
        throw ValidationError(
            fmt::format("Source disk image file '{}' does not exist", source_path));
    ...
    if (!size_match.hasMatch())
        throw ValidationError(
            fmt::format("Could not determine virtual size of disk image '{}'", source_path));
...
mp::BlockDevice::UPtr mp::QemuBlockDeviceFactory::load_block_device(
    const std::string& name,
...
mp::Path mp::QemuBlockDeviceFactory::get_block_device_path(const std::string& name,
                                                           const Path& data_dir) const
{
...
void mp::QemuBlockDeviceFactory::validate_name(const std::string& name) const
{
    ...
    if (!name_regex.match(QString::fromStdString(name)).hasMatch())
        throw ValidationError(fmt::format("Invalid block device name '{}'. Names must start with a "
