      ...
      tag{mp::utils::make_uuid(target).remove("-").left(30).prepend('m').toStdString()}
{
    ...
    if (mount_spec.uid_mappings.size() > 1 || mount_spec.gid_mappings.size() > 1)
    ...
    const auto uid_map = mount_spec.uid_mappings.empty() ? std::make_pair(1000, 1000) : mount_spec.uid_mappings[0];
    const auto gid_map = mount_spec.gid_mappings.empty() ? std::make_pair(1000, 1000) : mount_spec.gid_mappings[0];
