...
mp::DHCPReleaseProcessSpec::DHCPReleaseProcessSpec(const QString& bridge_name, const mp::IPAddress& ip, const QString& hw_addr)
    : bridge_name(bridge_name), ip(ip), hw_addr(hw_addr)
{
}

QString mp::DHCPReleaseProcessSpec::program() const
{
    return QStringLiteral("dhcp_release");
}

QStringList mp::DHCPReleaseProcessSpec::arguments() const
{
    return QStringList() << bridge_name << QString::fromStdString(ip.as_string()) << hw_addr;
}

QString mp::DHCPReleaseProcessSpec::apparmor_profile() const
{
    return "TODO!";
}
