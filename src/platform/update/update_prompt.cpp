...
mp::UpdatePrompt::~UpdatePrompt() = default;

bool mp::UpdatePrompt::time_to_show()
{
...
void mp::UpdatePrompt::populate(mp::UpdateInfo* update_info)
{
    auto new_release = monitor->get_new_release();
    if (new_release)
    {
...
void mp::UpdatePrompt::populate_if_time_to_show(mp::UpdateInfo* update_info)
{
    if (time_to_show())
