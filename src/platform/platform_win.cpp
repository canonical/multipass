...
{
    // On modern Windows, this retrieves both Win32 and Winsock errors
    DWORD win_err = GetLastError();

    // Block based on libuv's uv_translate_sys_error in error.c
    switch (win_err)
    {
    ...
    case ERROR_RETRY:
    case WSAEWOULDBLOCK:
        return EAGAIN; // On many platforms EAGAIN == EWOULDBLOCK
    ...
    case ERROR_ADAPTER_HARDWARE_ERROR:
    ...
    case WSAEOPNOTSUPP:
        return ENOTSUP; // Note: Or EOPNOTSUPP depending on target standard
    ...
    default:
        return -1; // Unknown or unmapped Windows error code
    }
}

// new_ACL returns a new ACL unique ptr that retains all existing Hyper-V ACEs or NULL if no
// entries exist.
